...
async function main(): Promise<void> {
  ...
  const engine = new HedgeFundEngine({
    triggerConfig: {
      ...
      atrPeriod: parseInt(process.env.TRIGGER_ATR_PERIOD || '14', 10),
      atrMultiplier: parseFloat(process.env.TRIGGER_ATR_MULTIPLIER || '0.75'),
      volumeSmaPeriod: parseInt(process.env.TRIGGER_VOLUME_SMA_PERIOD || '20', 10),
      volumeSpikeMultiplier: parseFloat(process.env.TRIGGER_VOLUME_SPIKE_MULTIPLIER || '1.5'),
    },
